...
const Navbar1 = () => {
  ...
  const [showContentMenu, setShowContentMenu] = useState(false); // New state for content dropdown
  ...
  const toggleContentMenu = () => {
    setShowContentMenu(!showContentMenu); // Toggle content dropdown
  };
  ...
  const renderRoleBasedLinks = () => {
    ...
    links.push(
      ...
      <Link key="promptbar" to="/promptbar" className="navbar-item">IPC Dictionary</Link>
    );

    // Role-specific links
    if (role === 'civilian' || role === 'lawstudent' || role === 'lawyer') {
      links.push(
        <Link key="courtroom" to="/courtroom" className="navbar-item">CourtRoom</Link>
      );
    }
